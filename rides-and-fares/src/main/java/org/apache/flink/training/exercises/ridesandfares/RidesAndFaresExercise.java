...
public class RidesAndFaresExercise {
    ...
    public static class EnrichmentFunction
        ...
        public void open(Configuration config) {
            ValueStateDescriptor<RideAndFare> desc = new ValueStateDescriptor<>("rideAndFare", RideAndFare.class, new RideAndFare());
            rideAndFare = getRuntimeContext().getState(desc);
        }

        @Override
        public void flatMap1(TaxiRide ride, Collector<RideAndFare> out) throws Exception {
            TaxiFare fare = rideAndFare.value().fare;
            if (fare != null) {
                out.collect(new RideAndFare(ride, fare));
                rideAndFare.clear();
            } else {
                rideAndFare.update(new RideAndFare(ride, null));
            }
        }

        @Override
        public void flatMap2(TaxiFare fare, Collector<RideAndFare> out) throws Exception {
            TaxiRide ride = rideAndFare.value().ride;
            if (ride != null) {
                out.collect(new RideAndFare(ride, fare));
                rideAndFare.clear();
            } else {
                rideAndFare.update(new RideAndFare(null, fare));
            }
        }
    }
}
